...
void inProReset(Node* head, int pri, int index){
  Node* check = head;
  while(check != NULL){
    if(check->priority == pri  && check->arrival <= index && check->inPro == -1){
      check->inPro = 0;
...
Node* findJob(Node* head, int index, int priority){
  if(head != NULL){
    Node* check = head;
    Node* pri = NULL;
    int count = 0;
    while(check != NULL){
      if(check->inPro == 0 && check->arrival <= index && priority == check->priority){
        pri = check;
        break;
      }
      if(check->inPro == -1 && check->arrival <= index &&  priority == check->priority){
        count++;
      }
      check = check->next;
    }
    ...
    if(count > 0){
      inProReset(head, priority, index);
      return findJob(head, index, priority);
    }else if(priority < 11){
      return findJob(head, index, ++priority);
    }
  }
...
void cycles(Node* head, int index, int work[], int cpuNum, int fin){
  int i, amount, count;
  Node* check;
  Node* pri[cpuNum];
  for(i = 0; i < cpuNum; i++){
  ...
  i = 0;
  for(i = 0; i < cpuNum; i++){
    // printf("CPU %d in process\n", (i+1));
    if(pri[i] == NULL){
      // printf("CPU %d is NULL C2\n", (i+1));
    }
    if(pri[i] != NULL){
      if(pri[i]->duration > 0){
  ...
  if(head != NULL || fin == 0){
    if(head == NULL) fin = 1;
    cycles(head, ++index, work, cpuNum, fin);
  }
}
